  ...
  return (
    <header className="h-14 shrink-0 border-b border-border bg-card flex items-center px-4 gap-4">
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <span className="text-xs text-muted-foreground font-mono">{data.theme.primary_color}</span>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {!isSaving && lastSaved && (
          <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <Check className="w-3.5 h-3.5 text-accent" /> Đã lưu {lastSaved.toLocaleTimeString("vi-VN")}
          </span>
        )}
        <Button
          onClick={onSave}
