...
const HeaderSection = ({ color }: { color: string }) => (
  <div className="text-center pb-5 mb-5" style={{ borderBottom: `2px solid ${color}` }}>
    ...
    <p className="text-sm text-slate-500 mt-1 font-medium">Frontend Developer</p>
  ...
  <div className="mb-5">
    ...
    <div className="ml-5">
      <div className="flex justify-between items-baseline">
        <h3 className="text-xs font-semibold text-slate-800">Đại học Bách Khoa TP.HCM</h3>
        <span className="text-[10px] text-slate-400">2017 – 2021</span>
      </div>
      <p className="text-xs text-slate-500 mt-0.5">Kỹ sư Khoa học Máy tính — GPA: 3.45/4.0</p>
    </div>
  </div>
);
...
const ExperienceSection = ({ color }: { color: string }) => (
  <div className="mb-5">
    ...
    <div className="ml-5 space-y-3">
      <div>
        <div className="flex justify-between items-baseline">
          <h3 className="text-xs font-semibold text-slate-800">Senior Frontend Developer</h3>
          <span className="text-[10px] text-slate-400">2022 – Hiện tại</span>
        </div>
        <p className="text-[10px] text-slate-400 font-medium">Công ty ABC Technology</p>
      ...
      <div>
        <div className="flex justify-between items-baseline">
          <h3 className="text-xs font-semibold text-slate-800">Junior Developer</h3>
          <span className="text-[10px] text-slate-400">2021 – 2022</span>
        </div>
        <p className="text-[10px] text-slate-400 font-medium">Startup XYZ</p>
        <ul className="mt-1 space-y-0.5 text-xs text-slate-600 list-disc list-inside">
    ...
    <div className="ml-5 grid grid-cols-2 gap-x-6 gap-y-1.5">
      {[
        ...
        <div key={skill.name} className="flex items-center gap-2">
          <span className="text-xs text-slate-600 w-28 shrink-0">{skill.name}</span>
...
const CvPreviewCanvas = () => {
  ...
  return (
    <div className="flex-1 overflow-auto bg-muted/30 p-8 flex justify-center">
      <div
        className="bg-white shadow-2xl rounded-sm w-full max-w-3xl mx-auto"
        style={{
          aspectRatio: "210 / 297",
